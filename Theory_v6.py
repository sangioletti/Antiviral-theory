...
data = { 'kEff': 4.0,
	 'x0' : 0,
         ...
         'maxDG' : 15,
         'epsilon_self' : 10**(-7),
         'nIntSamples' : 20, #Nr of points to sample numerical integral
         'countBeta' : 1000,
         'rhoNP' : 0.001,
         'verbose' : False, 
         'cV0' :  10**(-6),  #This is the molar concentration of viruses
         'cNP0' : 10**(-9),  #This is the molar concentration of nanoparticles 
...
def versor( r, z, direction ):
  ...
  ##Pain in the ass to be able to modify array without using if above
  ##so that it can be done vectorially
  #
  ##Make sure z is interpreted as array
  #z = np.array( z )
  #zOrig = np.array( z )
  #
  ##Select elements that are exactly zero and set them equal to pi/2.
  ##Remember that mask True means those elements are not seen
  #mask = ( z > 0.0 ) 
  #z = ma.masked_array( z, mask = mask )
  #z += np.pi / 2
  #
  ##Select all the other elements and set it equal to arctan r/z 
  ##Remember that mask True means those elements are not seen
  #mask = ( zOrig == 0.0 ) 
  #
  #z = ma.masked_array( z, mask = mask )
  ...
  ##Now you can combine into original array
  #angle = np.array( z )
...
def forceBound( r, direction, data ):
  ...
  myForce = np.zeros( len( zSamples ) )
  myPBound = np.zeros( len( zSamples ) )

  for i in range( len( zSamples ) ):
    myForce[ i ] = forceSingle( r, zSamples[ i ], direction, data )
...
def pCondBound( z, data ):
  ...
  den = Omega( data )
  kbT = data[ 'kbT' ]
  unnormalised = np.exp( -A( z, data ) / kbT )
