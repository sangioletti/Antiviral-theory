...
Replicate Figure 2 from the paper:
  Radial force F_r* vs binding strength DG0 for different N_L,
  in three panels for sigma*_R = 0.06, 0.60, 6.00.

Parameters (consistent with Run_Fstar_vs_NL.py):
...
# ── Main sweep ───────────────────────────────────────────────────────
# results[(sigma_star, NL)] = (DG0_list, Fstar_list)
results = {}
        ...
        for DG0 in DG0_values:
            ...
            try:
                with warnings.catch_warnings(record=True) as w:
                    ...
                    for warning in w:
                        print(f"  [WARN DG0={DG0:.1f}] {warning.message}")

                Fstar = Fr * R_ee / kbT
                print(f"  DG0={DG0:6.1f}  F_r={Fr:+.6e}  F*={Fstar:+.6e}")
...
for ext in ('pdf', 'png'):
    outpath = os.path.join(outdir, f"Fig2_diff_rigidity.{ext}")
