#!/usr/bin/env python3
"""
Variant of Run_Fig5.py:
  Fix Delta G0 = -14 kbT and sweep N_L on the x-axis (1 to 30).
  Plot three panels: |F_r*|, |F_z*|, |F*| vs N_L,
...
# ── Main sweep ───────────────────────────────────────────────────────
# results[ratio] = (NL_list, Fr_star_list, Fz_star_list, Ftot_star_list)
results = {}
...
for ratio in Nrep_ratios:
    ...
    for NL in NL_values:
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                ...
                for warning in w:
                    print(f"  [WARN NL={NL}] {warning.message}")

            Fr_star = Fr * R_ee / kbT
            Fz_star = Fz * R_ee / kbT
            Ftot_star = np.sqrt(Fr_star**2 + Fz_star**2)
            print(f"  NL={NL:3d}  Nrep={Nrep:3d}  Fr*={Fr_star:+.6e}  Fz*={Fz_star:+.6e}  |F*|={Ftot_star:.6e}")
        except Exception as e:
            print(f"  NL={NL:3d}  Nrep={Nrep:3d}  ERROR: {e}")
            Fr_star = np.nan
            Fz_star = np.nan
            Ftot_star = np.nan
    ...
    results[ratio] = (NL_values.tolist(), Fr_list, Fz_list, Ftot_list)
...
for panel_idx, ax in enumerate(axes):
    for idx, ratio in enumerate(Nrep_ratios):
        NLs, Fr_list, Fz_list, Ftot_list = results[ratio]
...
for ext in ('pdf', 'png'):
    outpath = os.path.join(outdir, f"Fig5_B.{ext}")
